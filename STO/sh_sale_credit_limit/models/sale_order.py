        ...
        if self._context.get('active_id', False) and self._context.get('active_model', False) == 'sale.order':
            ...
            if sale_obj:
                ...
                so_pend_obj = self.env['sale.order'].search(
                    [('state', 'not in', ['sale', 'cancel']), ('partner_id', '=', sale_obj.partner_id.id)])
...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):  # Overwrite Existing
        ...
        if self.state == 'on_hold' and self.env.user.has_groups('sh_sale_credit_limit.sh_group_sale_order_partner_credit_limit'):
    ...
    # Override standard method and add on_hold state
    def _confirmation_error_message(self):
        """ Return whether order can be confirmed or not if not then returm error message. """
        self.ensure_one()
        if self.state not in {'draft', 'sent','on_hold'}:
